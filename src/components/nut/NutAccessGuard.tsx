...
export function NutAccessGuard({ nutToken, children }: { nutToken: string; children: ReactNode }) {
  const { state, associate, offer, refresh } = useNutAccess(nutToken)
  const [recognized, setRecognized] = useState(false)
  const [confirming, setConfirming] = useState(false)
  const [offered, setOffered] = useState(false)
  const doAssociate = async () => { await associate(); setRecognized(true) }
  const doOffer = async () => { await offer(); setConfirming(false); setOffered(true) }
  ...
  return <>
    ...
    {state.status === 'error' && <AccessScreen title="Nao ne parvient pas à ouvrir le passage." text="Réessayez en approchant de nouveau la noix." action={<button className="quiet" onClick={() => void refresh()}>Réessayer</button>} />}
    {state.status === 'free' && !offered && <AccessScreen eyebrow="libre" title="Cette noix est libre." text="Vous pouvez l’associer à ce téléphone pour commencer votre traversée." action={<button className="primary" onClick={() => void doAssociate()}>Associer Nao</button>} />}
    ...
    {state.status === 'mine' && !recognized && <div className="nut-story"><div className="nut-recognition"><span>Nao vous reconnaît.</span><button onClick={() => document.querySelector<HTMLButtonElement>('.intro .primary')?.click()}>Poursuivre la traversée</button></div>{children}<button className="offer-nao" onClick={() => setConfirming(true)}>Offrir Nao</button></div>}
    {confirming && <div className="nut-modal" role="dialog" aria-modal="true" aria-labelledby="offer-title"><div><h2 id="offer-title">Offrir Nao ?</h2><p>Votre téléphone ne sera plus associé à cette noix.</p><p>La personne qui recevra Nao pourra l’associer à son téléphone et commencer sa propre traversée.</p><footer><button className="quiet" onClick={() => setConfirming(false)}>Annuler</button><button className="primary" onClick={() => void doOffer()}>Offrir Nao</button></footer></div></div>}
