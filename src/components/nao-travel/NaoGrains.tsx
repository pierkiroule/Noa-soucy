...
export function NaoGrains({ nutId, onShowBook, onRestart }: { nutId: string; onShowBook: () => void; onRestart: () => void }) {
  ...
  return <NaoScreen className="grains-experience"><Brand/><span className="eyebrow">Après l’épilogue</span><h1>Les 3 grains de sel•°</h1><div className="grains-narrative"><p>La traversée s’achève.</p><p>Et pourtant, certaines choses restent.</p><p>Un détail.<br/>Une sensation.<br/>Un son.<br/>Une image.<br/>Une présence…</p><p>Quels sont les trois grains de sel qui te restent de cette traversée&nbsp;?</p></div><h2>Choisis jusqu’à trois mots qui résonnent encore.</h2><div className="grain-field">{words.map(word => <NaoGrainTag key={word} selected={selected.includes(word)} disabled={selected.length >= 3 && !selected.includes(word)} onClick={() => toggle(word)}>{word}</NaoGrainTag>)}</div><button className="add-grain" onClick={() => setCustomOpen(open => !open)} aria-expanded={customOpen}>+ Ajouter mon grain de sel sensoriel</button>{customOpen && <div className="custom-grain"><input className="nao-input" maxLength={80} value={custom} placeholder="lumière sur l’eau" aria-label="Mon grain de sel sensoriel" onChange={event => { setCustom(event.target.value); setCustomError('') }} onKeyDown={event => { if (event.key === 'Enter') addCustom() }}/><button className="quiet" onClick={addCustom}>Ajouter</button>{customError && <small role="alert">{customError}</small>}</div>}{selected.length > 0 && <section className="grain-choice"><span>Tes grains de sel</span><strong>{selected.join(' · ')}</strong><p>Voilà ce que la traversée a laissé en toi.</p><p>Tu peux garder ces grains pour toi,<br/>ou les déposer dans le Livre d’Or•° de Nao.</p><button className="primary" onClick={() => setStep('signature')}>Déposer mes grains</button><button className="quiet" onClick={() => setStep('kept')}>Les garder pour moi</button></section>}<section className="grain-choice trace-choice"><button className="primary" onClick={leaveJustATrace}>Laisser juste une trace</button><button className="quiet" onClick={() => continueWithoutNaoTrace(() => setStep('kept'))}>Continuer sans laisser de trace</button></section></NaoScreen>
}
