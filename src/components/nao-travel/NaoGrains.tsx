...
export function NaoGrains({ nutId, onShowBook, onRestart }: { nutId: string; onShowBook: () => void; onRestart: () => void }) {
  ...
  if (step === 'kept' || step === 'deposited') return <main className="story grains-finale"><span className="eyebrow">Fin de traversée</span>{step === 'deposited' && <><h1>{selected.length ? 'Tes grains ont rejoint ceux des autres.' : 'Ta trace a rejoint celles des autres.'}</h1><p>Nao emporte désormais un peu de ta traversée.</p></>}<div className="transmission"><strong>Referme sa coquille.<br/>Confie-la à quelqu’un.</strong><p>Et laisse le voyage continuer.</p></div>{step === 'deposited' && <button className="primary" onClick={onShowBook}>Voir le Livre d’Or•°</button>}<button className="quiet" onClick={onRestart}>Recommencer le conte</button></main>
  if (step === 'signature') return <main className="story grains-experience signature"><span className="eyebrow">Les 3 grains de sel•°</span><h1>{selected.length ? 'Laisse une petite trace avec eux.' : 'Laisse juste une trace.'}</h1>{selected.length > 0 && <p className="selected-grains">{selected.join(' · ')}</p>}<form onSubmit={deposit}><label>Prénom, nom ou pseudo<input autoFocus maxLength={60} value={displayName} onChange={event => { setDisplayName(event.target.value); setStatus('idle') }} required/></label><label>Un coin du monde <small>facultatif</small><input maxLength={100} placeholder="Nantes, Rezé, Pays de Retz…" value={locationLabel} onChange={event => setLocationLabel(event.target.value)}/></label>{status === 'error' && <p className="travel-error" role="alert">Impossible de laisser cette trace pour le moment. Vérifie ta signature ou réessaie plus tard.</p>}<button className="primary" disabled={status === 'sending'}>{status === 'sending' ? 'La trace voyage…' : selected.length ? 'Déposer mes grains' : 'Signer le Livre d’Or•°'}</button><button type="button" className="quiet" onClick={() => setStep('choose')}>Retour</button></form></main>
