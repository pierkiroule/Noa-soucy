...
export function NaoPasserOnboarding({ nutId, onContinue }: { nutId: string; onContinue: () => void }) {
  ...
  const locate = () => {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      // Deliberately coarse and ephemeral: these values never leave this component.
      void Math.round(position.coords.latitude)
      void Math.round(position.coords.longitude)
      setPositionFound(true)
      setLocating(false)
      document.getElementById('nao-location')?.focus()
  ...
  return <main className="travel-screen"><section className="travel-card"><p className="eyebrow">Le voyage de Nao</p><h1>Nao vient d’arriver chez toi.</h1><p>Cette petite noix voyage de main en main.<br/>Tu peux laisser une trace de ton passage avant de commencer.</p>
    <form onSubmit={submit}>
      ...
      <div className="travel-location"><button className="quiet" type="button" onClick={locate} disabled={locating}>{locating ? 'Nao cherche ta zone…' : 'Me localiser'}</button>{positionFound && <span role="status">Position trouvée</span>}</div>
      ...
      <small>Ta localisation approximative sera visible par les prochains passeurs.</small>
