...
export function CinematicActPlayer({ act, onComplete, onPrevious }: { act: ActModule; onComplete: () => void; onPrevious?: () => void }) {
  ...
  return <article className="screen cinematic"><p className="eyebrow">Le vieux marin raconte</p><h2>{act.title}</h2>{videoAvailable && <video ref={videoRef} className="act-video" src={mediaUrl(act.media?.video)} poster={mediaUrl(act.media?.poster)} playsInline preload="metadata" onEnded={act.media?.voice ? undefined : onComplete} onError={() => { console.warn(`Vidéo indisponible : ${act.media?.video}`); setVideoAvailable(false) }} />}{(!videoAvailable || !started) && <div className="story-text">{act.text}</div>}<div className="media-controls">{!started || paused ? <button className="button" onClick={play}>{paused ? 'Reprendre' : 'Écouter et regarder'}</button> : <button className="button secondary" onClick={pause}>Pause</button>}<button className="text-button" onClick={restart}>Recommencer l’acte</button></div><StoryControls onNext={onComplete} onPrevious={onPrevious} /></article>
}
