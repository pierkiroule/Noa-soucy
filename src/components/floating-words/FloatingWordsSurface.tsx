...
export function FloatingWordsSurface({ onExit, onRestartStory }: { onExit: () => void; onRestartStory: () => void }) {
  ...
  const restart = useCallback(() => {
    ...
    setPetals(initializeWordPetals(rect?.width ?? innerWidth, rect?.height ?? innerHeight, reducedMotion, String(Date.now())))
  }, [reducedMotion])
  ...
  useEffect(() => {
    const animate = (now: number) => {
      const rect = surfaceRef.current?.getBoundingClientRect()
      if (document.visibilityState !== 'hidden' && rect) setPetals(current => stepWordPetals(current, Math.min(40, now - lastFrameRef.current), rect.width, rect.height, now, reducedMotion))
