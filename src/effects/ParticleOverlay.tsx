...
export function ParticleOverlay({visibleText,audioValues,config,sceneId,textRef}:{visibleText:string;audioValues:AudioReactiveValues;config:ParticleFxConfig;sceneId:string;textRef:React.RefObject<HTMLElement|null>}){
  ...
  useEffect(()=>{
    ...
    const draw=(p:Particle,family:ParticleFamily,alpha:number,w:number,h:number,time:number)=>{const active=contextRef.current,audio=valuesRef.current[active.audioBand],size=p.size*(.9+audio*.2),inside=textRef.current&&isInsideTextExclusionZone(p.x,p.y,textRef.current.getBoundingClientRect());ctx.globalAlpha=alpha*(.75+audio*.35)*(inside?1/3:1)*p.life;ctx.strokeStyle=ctx.fillStyle=colors[family][Math.floor(p.phase)%colors[family].length]
      ...
      else {ctx.beginPath();ctx.ellipse(p.x,p.y,size,family==='droplets'?size*1.4:size*.75,0,0,6.28);if(family==='droplets')ctx.stroke();else ctx.fill()}
