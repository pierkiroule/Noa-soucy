...
const ResonanceSeedsFlow = lazy(() => import('../components/resonance-seeds/ResonanceSeedsFlow').then(module => ({ default: module.ResonanceSeedsFlow })))
...
export function StoryPlayer() {
  ...
  const completeMedia = () => {
    ...
    else if (block.type === 'epilogue') setState(current => story.blocks[current.currentBlockIndex + 1]?.type === 'resonance-seeds' ? ({ ...current, currentBlockIndex: current.currentBlockIndex + 1, currentBreathIndex: 0 }) : ({ ...current, completed: true }))
  ...
  if (state.completed) return <main className="story completion"><ViewTransition variant="seeds"/><Brand/><span className="eyebrow">Fin de traversée</span><h1>Votre traversée s’arrête ici pour aujourd’hui.</h1><p>Les mots peuvent continuer de flotter.<br/>Sans réponse attendue.<br/>Sans chemin imposé.</p><div className="completion__actions"><button className="quiet" onClick={() => { setState(current => ({ ...current, completed: false, currentBlockIndex: story.blocks.findIndex(part => part.type === 'resonance-seeds') })); setStarted(true) }}>Graines de résonance</button><button className="primary" onClick={restart}>Recommencer le conte</button><button className="quiet" onClick={() => setStarted(false)}>Revenir à l’accueil</button></div></main>

  if (block?.type === 'resonance-seeds') return <Suspense fallback={<LoadingExperience label="Les graines rejoignent le vent…"/>}><ResonanceSeedsFlow /></Suspense>
