...
import { FlyingBubblesFlow } from '../components/flying-bubbles/FlyingBubblesFlow'
...
export function StoryPlayer() {
  ...
  const completeMedia = () => {
    ...
    else if (block.type === 'epilogue') setState(current => story.blocks[current.currentBlockIndex + 1]?.type === 'flying-bubbles' ? ({ ...current, currentBlockIndex: current.currentBlockIndex + 1, currentBreathIndex: 0 }) : ({ ...current, completed: true }))
  ...
  if (block?.type === 'flying-bubbles') return <FlyingBubblesFlow onExit={() => setState(current => ({ ...current, completed: true }))} onRestartStory={restart} />
...
function ScoreReader({ story, onClose }: { story:StoryDocument; onClose:()=>void }) {
  const entries = story.blocks.flatMap(block => block.type === 'question' ? [block, ...block.choices.map(choice => choice.resonance)] : block.type === 'flying-bubbles' ? [] : [block])
