...
export function SceneStage({scene,media,parameters,effects,time,onMediaError}:Props){const Placeholder=sceneRegistry[scene.media.fallbackSceneId];return <div className="stage" data-scene={scene.id}>
 <div className="stage__media"><Placeholder time={time} parameters={parameters} effects={effects}/>{media?.kind==='video'&&<video className="stage__video" src={media.source} autoPlay muted loop playsInline onError={onMediaError}/>}</div>
