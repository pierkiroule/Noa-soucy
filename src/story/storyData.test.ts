...
test('story video follows the fixed 1 to 14 mapping with one continuous soundtrack', () => {
  const media = story.blocks.flatMap(block => block.type === 'question' ? block.choices.map(choice => choice.resonance.media) : block.type === 'navigation-flower' ? [] : [block.media])
...
test('story keeps the navigation flower optional after the epilogue', () => {
  ...
  assert.deepEqual(story.blocks[epilogueIndex + 1], {
    id: 'navigation-flower-main',
    type: 'navigation-flower',
    module: 'navigation-flower-main',
    title: 'La Fleur-Boussole des navigateurs de l’incertitude',
    enabled: true,
    optional: true
  })
  assert.equal(story.navigationFlower?.['navigation-flower-main']?.optional, true)
})
