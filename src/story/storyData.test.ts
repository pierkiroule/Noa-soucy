...
test('story video follows the fixed 1 to 14 mapping with one continuous soundtrack', () => {
  const media = story.blocks.flatMap(block => block.type === 'question' ? block.choices.map(choice => choice.resonance.media) : block.type === 'resonance-seeds' ? [] : [block.media])
...
test('story opens Graines de résonance directly after the epilogue', () => {
  ...
  assert.deepEqual(story.blocks[epilogueIndex + 1], {
    id: 'resonance-seeds-main',
    type: 'resonance-seeds',
    module: 'resonance-seeds-main',
    title: 'Graines de résonance',
    enabled: true,
    optional: true
  })
})
...
test('resonance seeds block is ignored for media preloading', () => {
  const epilogueIndex = story.blocks.findIndex(block => block.type === 'epilogue')
