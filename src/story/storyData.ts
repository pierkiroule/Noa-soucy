...
export interface NavigationFlowerStoryboardBlock { id:string; type:'navigation-flower'; module:'navigation-flower-main'; title:string; enabled:boolean; optional:boolean }
...
export type StoryBlock = StoryMediaBlock | StoryQuestion | NavigationFlowerStoryboardBlock
export interface StoryDocument { version:number; id:string; title:string; subtitle:string; blocks:StoryBlock[]; navigationFlower?:Record<string,{ id:string; title:string; enabled:boolean; optional:boolean }> }
...
export function getNextMedia(blocks: StoryBlock[], index: number): StoryMedia | undefined {
  ...
  return !next || next.type === 'question' || next.type === 'navigation-flower' ? undefined : next.media
}
