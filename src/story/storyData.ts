...
export interface ResonancesStoryboardBlock { id:string; type:'resonances'; module:'metaphoric-resonances'; title:string; enabled:boolean; optional:boolean }
...
export type StoryBlock = StoryMediaBlock | StoryQuestion | ResonancesStoryboardBlock
export interface StoryDocument { version:number; id:string; title:string; subtitle:string; blocks:StoryBlock[]; resonances?:Record<string,{ id:string; title:string; enabled:boolean; optional:boolean }> }
...
export function getNextMedia(blocks: StoryBlock[], index: number): StoryMedia | undefined {
  ...
  return !next || next.type === 'question' || next.type === 'resonances' ? undefined : next.media
}
